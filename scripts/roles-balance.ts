  ...
  for (let stage = 0; stage < stages + (stages === 12 ? afterHours : 0); stage += 1) {
    ...
    while (run.phase === 'ready' || run.phase === 'lost') {
      if (run.phase === 'lost') {
        ...
        run = retryCommission(run, acceptRetryHelp);
        build();
      }
      ...
      starved += result.events.filter((event) => event.blocked).length;
      timeouts += Number(result.timedOut);
      run = settleDrop(launchDrop(run), result);
      if (run.phase === 'ready') build();
    }
    records.push({ stage: run.stage + 1, target: commission(run).target, capacity: commission(run).capacity, won: run.phase === 'review', drops: run.totalDrops - startingDrops, retries: run.retries, retryHelp: retryHelpLevel(run), power: run.power, credits: run.brass, best, board: structuredClone(run.board), bench: structuredClone(run.bench), banked, joins, starved });
...
export function auditRoleRecovery(lost: RunState, evaluate = createTrialEvaluator('baseline').evaluate) {
  ...
  return {
    ...
    limitation: 'Detached whole-board search using owned parts and unchanged chosen help. No automatic assistance; failure to find a build is not proof of impossibility.',
    run: rebuilt.run,
  };
}
...
async function main() {
  ...
  await writeFile(`${stem}.json`, JSON.stringify({ generatedAt: new Date().toISOString(), seeds, policies, continuation, retryPolicy: values['retry-help'] ? 'explicitly-accept-help' : 'same-difficulty', rules: 'charge-tuning-ten-parts-optional-help', runs }, null, 2), { flag: 'wx' });
