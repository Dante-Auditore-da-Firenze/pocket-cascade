...
| R04 | Commit and publish to origin | Open: public origin ready; initial commit/push next | GitHub login confirmed as Dante-Auditore-da-Firenze. Created the remote and configured origin. GitHub rejected private-repository rules on the current plan; the user explicitly selected making the repository public and enabling the rules. Two rulesets are active and verified. Commit author is configured locally with the GitHub noreply email. Restage/rescan the final files, create the short `Initial commit`, then verify the push. See HISTORY.md H024. |
| R05 | Require owner approval for main, with temporary owner bypass | Open: server rules active; awaiting uploaded CODEOWNERS check | User first requested no direct main pushes, then explicitly allowed their own account to push directly for now. The PR ruleset requires one code-owner approval and dismisses stale approvals, with only GitHub user ID 73032811 allowed to bypass it. A separate ruleset blocks force pushes and deletion without bypasses. Both active rulesets were read back from GitHub; the all-files CODEOWNERS entry must still be verified on the published main branch. See HISTORY.md H024. |
