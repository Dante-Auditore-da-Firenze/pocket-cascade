...
The browser configuration starts Vite on `http://127.0.0.1:5174`, separate from
the player's 5173 server. `POCKET_TEST_PORT` can override it; strict port selection
prevents silently testing a different process. Native tests load the built
renderer directly, so rebuild first. No test should reset the user's live game.
...
| [tests/balance.test.ts](../tests/balance.test.ts) | Legal placement, early purchases, a full fresh seed-42 role-aware campaign with earned tuning/fusion and needed middle-campaign edits, detached observations, frozen-layout isolation, and bounded local search. Passing does not establish human enjoyment or universal difficulty. |
...
- [tests/browser/rewards.spec.ts](../tests/browser/rewards.spec.ts) covers three viewport sizes, one-use rewards, selected-copy tuning, paid tuning, fusion/undo safety, full storage, import/reload, and After Hours slot expansion. Boundary fixtures are distinguished from the first shop earned through actual play.
